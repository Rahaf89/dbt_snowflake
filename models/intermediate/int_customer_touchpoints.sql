...
touchpoints_before_order as (
    select
        ...
        e.event_timestamp,
        row_number() over (
            partition by o.order_id order by e.event_timestamp asc
        ) as touch_position_asc,
        row_number() over (
            partition by o.order_id order by e.event_timestamp desc
    ...
    inner join events e
        ...
        and e.event_timestamp >= {{ dbt_utils.dateadd('day', -1 * var('attribution_window_days'), 'o.order_timestamp') }}
)

select * from touchpoints_before_order
